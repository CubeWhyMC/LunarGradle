...
import org.cubewhy.gradle.utils.DownloadUtils
import org.cubewhy.gradle.utils.AccessWidener
...
/**
 * Adds Minecraft as a dependency by providing the jar to the project file tree.
 */
private fun Project.addMinecraftAssets(version: MinecraftVersion) {
    val manifest = DownloadUtils.fetch(Constants.VERSION_MANIFEST).decodeJSON<VersionManifest>() ?: return
    val versionEntry = manifest.versions.find { it.id == version.version } ?: return
    val versionInfo = DownloadUtils.fetch(versionEntry.url).decodeJSON<VersionInfo>() ?: return

    val client = versionInfo.downloads.client
    DownloadUtils.downloadAndChecksum(URL(client.url), client.sha1, version.minecraftJar.toPath())
...
private fun Project.addMappedMinecraft(version: MinecraftVersion) = runCatching {
    val mapped = File(Constants.CACHE_DIR, "minecraft-mapped-${version.version}.jar")
    if (mapped.exists()) {
        dependencies.add("compileOnly", files(mapped))
        return@runCatching
    }

    val remapper = createRemapper(version)

    JarFile(version.minecraftJar).use { mcJar ->
        JarOutputStream(mapped.outputStream()).use { outputStream ->
            for (entry in mcJar.entries()) {
                ...
                reader.accept(AccessWidener(ClassRemapper(cw, remapper as Remapper)), 0)
